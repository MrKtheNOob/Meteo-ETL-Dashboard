...
-- Table: donnees_meteo_horaires_historiques
CREATE TABLE donnees_meteo (
    ...
    UNIQUE (id_lieu, datetime_observation), -- Assure une seule observation par lieu et par horodatage précis
...
-- (Ces index correspondent à des clés primaires ou uniques, ou sont ajoutés sur les FK)
CREATE INDEX idx_dmhh_id_lieu ON donnees_meteo_horaires_historiques(id_lieu);
CREATE INDEX idx_dmhh_datetime_observation ON donnees_meteo_horaires_historiques(datetime_observation);
